...
export async function GET(
  ...
  try {
    const expenseClient = (prisma as any).expense;
    if (!expenseClient) {
      return apiResponse({
        status: 500,
        message: "Prisma client non généré pour Expense. Lancez `prisma generate`."
      });
    }

    const expense = await expenseClient.findUnique({
      where: { id: params.id }
    });
...
export async function PATCH(
  ...
  try {
    ...
    const expense = await expenseClient.update({
      where: { id: params.id },
      data: body
    });
